function [W, J_hist, J_hist_all, primal_residual_val, primal, dual_residual_val, dual] = Least_SWMTL33(X, Y, opts)
...
function v = slackingFunVal(x, un, up, w, l, rho, lambda, c)
    v = c*(max(0,-un*x) + max(0,-up*x)) + lambda*abs(x) - l*x + (rho/2)*(x-w)^2;
end
...
function [J, grad] = computeGradients(X, Y, W, U, L, t, lambda, rho, c)
    ...
    grad = (1/m)*(yt_error'*xt)' + lagrange_grad;
end
